...
- SVM: F1=0.586, Precision=0.492, Recall=0.486
...
### Peak Performance (Google Colab - Single Run)
During experimentation on Google Colab with the same configuration (Window=170, Overlap=18, Lowpass 5Hz), one particular run achieved exceptional performance:
- **SVM Accuracy: 0.58** (58% across all 14 activities)
- This result was achieved only once and could not be consistently reproduced
- Likely due to random initialization, specific volunteer ordering in cross-validation, or computational environment differences
- Demonstrates the potential ceiling of the approach under optimal conditions

![Peak Performance Google Colab](images/colab_peak_performance_058.png)
*Figure: Classification report from Google Colab showing peak accuracy of 0.58 - achieved once during experimentation*
...
2. **Longer windows are better:** Window=170 (5.3 seconds) captures complete activity cycles
3. **Low overlap wins:** Overlap=18 (10%) beats 50% for both accuracy and efficiency
4. **SVM is the best classifier:** Consistently outperforms Random Forest and Decision Tree

**Reproducible Performance:** The final system consistently achieves 48.8% F1 score (0.488 accuracy) across 14 daily activities using Window=170, Overlap=18, and Lowpass 5Hz filtering.

**Peak Performance:** During one exceptional run on Google Colab, the same configuration achieved 58% accuracy (0.58), demonstrating the approach's potential ceiling. However, this result was not consistently reproducible across multiple runs.

The system demonstrates effective classification despite activity complexity and similarity between classes (e.g., climb_stairs vs descend_stairs, sitdown_chair vs standup_chair).
